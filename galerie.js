...
function renderGrid(photos) {
  ...
  if (!photos.length) {
    grid.innerHTML = '<p class="boutique-empty">Aucune photo n\'est encore associée à cette galerie. Revenez bientôt !</p>';
    return;
  }
...
async function unlockGallery(slug, code) {
  ...
  try {
    ...
    currentSlug = slug;
    sessionStorage.setItem('jme-gallery-slug', slug);
    sessionStorage.setItem('jme-gallery-code', code);

    document.getElementById('galleryTitle').textContent = data.name;
    document.title = `${data.name} – Jean-Michel Expert Photographe`;
    renderGrid(data.photos);

    document.getElementById('galleryLock').hidden = true;
    document.getElementById('galleryContent').hidden = false;
    document.getElementById('cartToggle').hidden = false;
  } catch (e) {
    errorEl.textContent = e.message;
  } finally {
    btn.disabled = false;
    btn.textContent = 'Accéder à ma galerie';
  }
}

document.getElementById('galleryUnlockBtn').addEventListener('click', () => {
  const slug = document.getElementById('gallerySlug').value.trim();
  const code = document.getElementById('galleryCode').value.trim();
  if (!slug || !code) {
    document.getElementById('galleryError').textContent = 'Merci de renseigner la galerie et le code.';
    return;
  }
  unlockGallery(slug, code);
});
document.getElementById('galleryCode').addEventListener('keydown', (e) => {
  if (e.key === 'Enter') document.getElementById('galleryUnlockBtn').click();
});
...
// Pré-remplissage / auto-déverrouillage depuis le lien envoyé au client (?g=slug)
// ou depuis une session déjà déverrouillée pendant cette visite.
const urlSlug = new URLSearchParams(location.search).get('g');
const savedSlug = sessionStorage.getItem('jme-gallery-slug');
const savedCode = sessionStorage.getItem('jme-gallery-code');

if (urlSlug) document.getElementById('gallerySlug').value = urlSlug;

if (savedSlug && savedCode && (!urlSlug || urlSlug === savedSlug)) {
  unlockGallery(savedSlug, savedCode);
} else if (urlSlug) {
